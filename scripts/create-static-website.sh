...
if [[ "$#" -lt 3 ]] || [[ "$#" -gt 4 ]]; then
  echo "Usage: ${0} <stack_type> <stack_prefix> <website> <config_path> "
  exit 1
fi
...
config_paths=()
IFS=':' read -ra temp_config_paths <<< "${4}"
...
log_path=logs/$stack_prefix/$run_id-${tag}-$(echo "$stack_prefix" | sed 's/\//-/g').log
...
extra_vars=(--extra-vars "stack_prefix=$stack_prefix  website=$website " )
